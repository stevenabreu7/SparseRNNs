# Copied from https://github.com/IntelLabs/IntelNeuromorphicDNSChallenge/blob/main/dnsmos.py

import os
from typing import Dict, List, Tuple

import numpy as np
# pip install onnxruntime-gpu # <--- to install gpu version
...
class DNSMOS:
    ...
    def __init__(
        ...
        self.session_options.intra_op_num_threads = 14
        self.session_options.inter_op_num_threads = 14
        self.session_options.execution_mode = ort.ExecutionMode.ORT_PARALLEL

        self.onnx_sess = ort.InferenceSession(
            "/export/share/datasets/dns_fullband/IntelNeuromorphicDNSChallenge/microsoft_dns/DNSMOS/DNSMOS/sig_bak_ovr.onnx",
            providers=providers,
    ...
    def get_polyfit_val(
        self, sig: float, bak: float, ovr: float
    ...
    def full_score(self, audio: np.ndarray) -> Dict[str, float]:
        ...
        num_hops = int(np.floor(len(audio) / self.sampling_rate) - INPUT_LENGTH) + 1
    ...
    def __call__(self, audio: np.ndarray) -> float:
        ...
        if len(audio.shape) == 1:
            ...
            return np.array([full_score["OVRL"], full_score["SIG"], full_score["BAK"]])
        elif len(audio.shape) == 2:
            batch_scores = []
            for b in range(audio.shape[0]):
                full_score = self.full_score(audio[b])
                batch_scores.append(
                    [full_score["OVRL"], full_score["SIG"], full_score["BAK"]]
                )
            return np.array(batch_scores)
        raise RuntimeError(
            f"Found unsupported audio of shape {audio.shape}"
