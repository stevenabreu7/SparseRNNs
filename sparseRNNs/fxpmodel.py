...
def fxp_relu(
    ...
    if isinstance(x, ComplexFxpArray):
        new_data = jax.nn.relu(x.real.data + 1j * x.imag.data)
...
def recurrent_loop(
    ...
    xt_scan = np.stack([xt_real, xt_imag], axis=-1)
...
class FxpSSM(FxpModule):
    ...
    def forward(
        ...
        if self.use_lax_scan:
            if Bu_elements.ndim == 3:
                vmap_in_axes = (0, 0, None, None, 0, 0, *(None,) * 6)
                recurrent_loop_fn = jax.vmap(recurrent_loop, in_axes=vmap_in_axes)
            else:
