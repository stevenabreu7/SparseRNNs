...
def make_DPLR_HiPPO(N):
    ...
    with jax.default_device(jax.devices('cpu')[0]):
        S_cpu = jax.device_put(S * -1j, jax.devices('cpu')[0])
        Lambda_imag, V = eigh(S_cpu)
    # Move results back to GPU (if that's your default device)
    Lambda_imag = jax.device_put(Lambda_imag, jax.devices('gpu')[0])
